combineresults<-function(data_date,Recovery_Goals,version='original',exclude='none'){ #ESU_DIP_list_all
  ...
  write.csv(Percentchange,paste(SubDir,"/Percent_change_",data_date,"_", version, "_excluded", exclude,".csv",sep=""),row.names = F)
  ...
  write.csv(Smoothed_Abundance,paste(SubDir,"/Smoothed_Abundance_",data_date,"_", version, "_excluded", exclude,".csv",sep=""),row.names = F)
}
